...
async function createTable(n) {
  const { error } = await supabase.from('tables').insert({ table_number: n })
  if (error) throw error
}
...
async function getNightWinners() {
  const { data, error } = await supabase.from('bets')
    ...
    .eq('night_date', getToday()).eq('is_correct', true).order('points_earned', { ascending: false })
  if (error) throw error
  return data
}
...
async function getTournamentStats() {
  ...
  return { players: players ?? [], totalBets: bets?.length ?? 0,
...
export default function AdminPage() {
  ...
  const [newMatch, setNewMatch] = useState({ home_team: '', away_team: '', home_flag: '', away_flag: '', group_name: '', match_date: '' })
  ...
  const createTableMut = useMutation({ mutationFn: createTable, onSuccess: () => { toast.success('Mesa creada'); qc.invalidateQueries({ queryKey: ['tables'] }) }, onError: (e) => toast.error('Error: ' + e.message) })
  const deleteTableMut = useMutation({ mutationFn: deleteTable, onSuccess: () => { toast.success('Mesa eliminada'); qc.invalidateQueries({ queryKey: ['tables'] }) }, onError: (e) => toast.error('Error: ' + e.message) })
  const addPrizeMut = useMutation({ mutationFn: addPrizeConfig, onSuccess: () => { toast.success('Premio configurado'); qc.invalidateQueries({ queryKey: ['prize-config'] }) }, onError: (e) => toast.error('Error: ' + e.message) })
  const removePrizeMut = useMutation({ mutationFn: removePrizeConfig, onSuccess: () => { toast.success('Premio eliminado'); qc.invalidateQueries({ queryKey: ['prize-config'] }) }, onError: (e) => toast.error('Error: ' + e.message) })
  const removeWaiterMut = useMutation({ mutationFn: removeWaiter, onSuccess: () => { toast.success('Mesero eliminado'); qc.invalidateQueries({ queryKey: ['waiters'] }) }, onError: (e) => toast.error('Error: ' + e.message) })
  const closeTableMut = useMutation({ mutationFn: closeTable, onSuccess: () => { toast.success('Mesa cerrada'); qc.invalidateQueries({ queryKey: ['tables'] }); qc.invalidateQueries({ queryKey: ['sessions'] }); setClosingTable(null) }, onError: (e) => toast.error('Error: ' + e.message) })
  const assignPrizeMut = useMutation({ mutationFn: assignPrize, onSuccess: () => { toast.success('Premio asignado'); setAssigningPrize(null); setPrizeDesc(''); qc.invalidateQueries({ queryKey: ['prizes'] }) }, onError: (e) => toast.error('Error: ' + e.message) })
  ...
  function handleLiveScore(matchId) {
    ...
    if (!s || s.home === '' || s.away === '' || s.home === undefined || s.away === undefined) return toast.error('Ingresa el marcador')
  ...
  function handleCreateMatch() {
    ...
    if (!home_team || !away_team || !match_date) return toast.error('Completa los campos')
    createMatchMut.mutate({ home_team, away_team, home_flag: home_flag || '🏳️', away_flag: away_flag || '🏳️', group_name: group_name || 'Amistoso', match_date: new Date(match_date).toISOString() })
  ...
  async function handleAddWaiter() {
    ...
    try {
      const { data: userId, error: rpcError } = await supabase.rpc('get_user_id_by_email', { p_email: waiterEmail.trim().toLowerCase() })
      if (rpcError) { toast.error('Error: ' + rpcError.message); return }
      if (!userId) { toast.error('No se encontró usuario con ese email'); return }
      const { error } = await supabase.from('profiles').update({ role: 'waiter' }).eq('id', userId)
      if (error) { toast.error('Error: ' + error.message); return }
      toast.success('Mesero agregado')
      qc.invalidateQueries({ queryKey: ['waiters'] }); qc.invalidateQueries({ queryKey: ['all-users'] })
      setWaiterEmail('')
  ...
  async function handleAddAdmin() {
    ...
    try {
      const { data: userId, error: rpcError } = await supabase.rpc('get_user_id_by_email', { p_email: adminEmail.trim().toLowerCase() })
      if (rpcError) { toast.error('Error: ' + rpcError.message); return }
      if (!userId) { toast.error('No se encontró usuario con ese email'); return }
  ...
  const todayMatches = useMemo(() => matches.filter(m => {
    const d = new Date(new Date(m.match_date).getTime() - 5 * 60 * 60 * 1000)
    ...
    <div className="min-h-screen bg-monaco-black">
      ...
      <div className="px-4 py-6 space-y-4">
        ...
        {tab === 'dashboard' && (
          <div className="space-y-5">
            {liveMatches.length > 0 && (
              <div>
                <p className="section-label flex items-center gap-2"><span className="w-2 h-2 bg-green-400 rounded-full animate-pulse" /> En vivo</p>
                {liveMatches.map(m => <LiveCard key={m.id} match={m} liveScore={liveScores[m.id]} onScoreChange={(s, v) => setLiveScores(p => ({ ...p, [m.id]: { ...p[m.id], [s]: v } }))} onUpdate={() => handleLiveScore(m.id)} onFinish={() => handleFinish(m.id)} isPending={liveScoreMut.isPending || finishMut.isPending} />)}
              </div>
            )}
                  ...
                  {stats.players.slice(0, 5).map((p, i) => (
                    <div key={p.id} className={`card flex items-center gap-3 ${i === 0 ? 'border-yellow-400/30 bg-yellow-400/5' : ''}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-monaco-silver text-xs">{p.total_correct} aciertos</p>
                      </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: 'Mesas', icon: LayoutGrid, tab: 'mesas', color: 'red' },
                ...
                <button key={item.tab} onClick={() => setTab(item.tab)} className="card flex items-center gap-3 p-3 active:scale-95 transition-all">
                  ...
                  <div className="flex-1 flex items-center justify-between min-w-0">
                    ...
                    <ChevronRight size={12} className="text-monaco-silver/40" />
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="space-y-4">
            <div className="card border-monaco-red/20 space-y-3">
              ...
              <div className="flex gap-2">
                <input type="number" min="1" value={newTableNum} onChange={e => setNewTableNum(e.target.value)} placeholder="Número" className="flex-1 bg-monaco-black border border-white/10 rounded-xl px-3 py-2.5 text-sm text-monaco-white placeholder-monaco-silver/40 focus:outline-none focus:border-monaco-red/50" />
            ...
            {activeTables.map(table => {
              const tu = sessions.filter(s => s.table_number === table.table_number)
              return (
                <div key={table.id} className="card border-monaco-red/20 space-y-3">
                  ...
                  {tu.map(s => (
                    <div key={s.id} className="flex items-center gap-2.5 pt-2 border-t border-white/5">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {freeTables.map(t => (
                    <div key={t.id} className="card aspect-square flex flex-col items-center justify-center gap-1 relative group">
                      ...
                      <span className="text-[9px] text-monaco-silver/40 uppercase">Libre</span>
        ...
        {tab === 'partidos' && (
          <div className="space-y-4">
            <div className="flex bg-monaco-card rounded-xl p-1 border border-white/5">
              <button onClick={() => setMatchView('today')} className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${matchView === 'today' ? 'bg-monaco-red text-white' : 'text-monaco-silver'}`}>📅 Hoy</button>
              <button onClick={() => setMatchView('all')} className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${matchView === 'all' ? 'bg-monaco-red text-white' : 'text-monaco-silver'}`}>📋 Todos</button>
            </div>
            <div className="card border-monaco-red/20 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[{ key: 'home_team', l: 'Local', ph: 'Colombia' }, { key: 'away_team', l: 'Visitante', ph: 'Brasil' }, { key: 'home_flag', l: 'Bandera local', ph: '🇨🇴' }, { key: 'away_flag', l: 'Bandera visitante', ph: '🇧🇷' }].map(f => (
                  <div key={f.key} className="space-y-1"><p className="text-[10px] text-monaco-silver uppercase">{f.l}</p><input value={newMatch[f.key]} onChange={e => setNewMatch(m => ({ ...m, [f.key]: e.target.value }))} placeholder={f.ph} className="w-full bg-monaco-black border border-white/10 rounded-xl px-3 py-2 text-sm text-monaco-white placeholder-monaco-silver/30 focus:outline-none focus:border-monaco-red/50" /></div>
                ))}
              </div>
              <div className="space-y-1"><p className="text-[10px] text-monaco-silver uppercase">Grupo</p><input value={newMatch.group_name} onChange={e => setNewMatch(m => ({ ...m, group_name: e.target.value }))} placeholder="Grupo A..." className="w-full bg-monaco-black border border-white/10 rounded-xl px-3 py-2 text-sm text-monaco-white placeholder-monaco-silver/30 focus:outline-none focus:border-monaco-red/50" /></div>
              <div className="space-y-1"><p className="text-[10px] text-monaco-silver uppercase">Fecha y hora</p><input type="datetime-local" value={newMatch.match_date} onChange={e => setNewMatch(m => ({ ...m, match_date: e.target.value }))} className="w-full bg-monaco-black border border-white/10 rounded-xl px-3 py-2 text-sm text-monaco-white focus:outline-none focus:border-monaco-red/50" /></div>
            ...
            {matchView === 'today' && (
              <div className="space-y-4">
                ...
                {todayLive.length > 0 && <div><p className="section-label flex items-center gap-2"><span className="w-2 h-2 bg-green-400 rounded-full animate-pulse" /> En vivo</p>{todayLive.map(m => <LiveCard key={m.id} match={m} liveScore={liveScores[m.id]} onScoreChange={(s, v) => setLiveScores(p => ({ ...p, [m.id]: { ...p[m.id], [s]: v } }))} onUpdate={() => handleLiveScore(m.id)} onFinish={() => handleFinish(m.id)} isPending={liveScoreMut.isPending || finishMut.isPending} />)}</div>}
            ...
            {matchView === 'all' && (
              <div className="space-y-4">
                {liveMatches.length > 0 && <div><p className="section-label flex items-center gap-2"><span className="w-2 h-2 bg-green-400 rounded-full animate-pulse" /> En vivo</p>{liveMatches.map(m => <LiveCard key={m.id} match={m} liveScore={liveScores[m.id]} onScoreChange={(s, v) => setLiveScores(p => ({ ...p, [m.id]: { ...p[m.id], [s]: v } }))} onUpdate={() => handleLiveScore(m.id)} onFinish={() => handleFinish(m.id)} isPending={liveScoreMut.isPending || finishMut.isPending} />)}</div>}
        ...
        {tab === 'premios' && (
          <div className="space-y-4">
            <div className="card border-monaco-red/20 space-y-3">
              ...
              <p className="text-monaco-silver text-xs">Se entrega al cliente con más puntos al finalizar.</p>
              <div className="space-y-2">
        ...
        {tab === 'apuestas' && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-2">
              <div className="card text-center border-monaco-red/30 bg-monaco-red/5"><p className="text-2xl font-display text-monaco-red">{allBets.length}</p><p className="text-[10px] text-monaco-silver tracking-widest uppercase mt-1">Apuestas</p></div>
              <div className="card text-center"><p className="text-2xl font-display text-monaco-white">{allBets.filter(b => b.is_correct).length}</p><p className="text-[10px] text-monaco-silver tracking-widest uppercase mt-1">Aciertos</p></div>
            </div>
            {allBets.length === 0 && <div className="card text-center py-10"><p className="text-monaco-silver text-sm">No hay apuestas</p></div>}
            {allBets.map(bet => {
              ...
              return (
                <div key={bet.id} className={`card mb-2 ${isExact ? 'border-green-500/30 bg-green-500/5' : isResult ? 'border-yellow-500/20 bg-yellow-500/5' : ''}`}>
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-8 h-8 rounded-full bg-monaco-red/20 border border-monaco-red/30 flex items-center justify-center text-xs text-monaco-red flex-shrink-0">{bet.profiles?.avatar_url ? <img src={bet.profiles.avatar_url} className="w-full h-full rounded-full object-cover" /> : bet.profiles?.full_name?.[0] ?? '?'}</div>
                    <div className="flex-1 min-w-0">
                    ...
                    {isFinished && <span className={`text-xs font-display flex-shrink-0 ${isExact ? 'text-green-400' : isResult ? 'text-yellow-400' : 'text-monaco-silver/50'}`}>+{bet.points_earned} pts</span>}
                  </div>
                  <div className="flex items-center justify-between bg-monaco-black/50 rounded-xl px-3 py-2">
                    <div className="flex items-center gap-2"><span>{match?.home_flag}</span><span className="text-monaco-white text-xs">{match?.home_team}</span></div>
                    <span className="text-monaco-red font-display text-lg">{bet.predicted_home} — {bet.predicted_away}</span>
                    <div className="flex items-center gap-2"><span className="text-monaco-white text-xs">{match?.away_team}</span><span>{match?.away_flag}</span></div>
                  </div>
                  {isFinished && (
        ...
        {tab === 'usuarios' && (
          <div className="space-y-4">
            <div className="card border-yellow-500/20 space-y-3">
              ...
              <p className="text-monaco-silver text-xs">El mesero debe registrarse primero.</p>
            ...
            <div className="card border-monaco-red/20 space-y-3">
              <p className="text-monaco-white text-sm font-medium flex items-center gap-2"><Crown size={14} className="text-monaco-red" /> Agregar administrador</p>
              <p className="text-monaco-silver text-xs">El usuario debe registrarse primero.</p>
              <input value={adminEmail} onChange={e => setAdminEmail(e.target.value)} placeholder="email@ejemplo.com" type="email" className="w-full bg-monaco-black border border-white/10 rounded-xl px-3 py-2.5 text-sm text-monaco-white placeholder-monaco-silver/40 focus:outline-none focus:border-monaco-red/50" />
              <button onClick={handleAddAdmin} className="w-full py-2.5 bg-monaco-red rounded-xl text-white text-sm font-medium">Asignar rol de admin</button>
            </div>
            {waiters.length > 0 && (
              <div>
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center px-6">
          <div className="bg-monaco-card rounded-3xl p-6 w-full max-w-xs border border-white/10 space-y-4">
            <div className="text-center">
              ...
              <p className="text-monaco-silver text-xs mt-2">Se bloquearán los usuarios.</p>
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={() => closeTableMut.mutate({ tableNumber: closingTable, closedBy: user.id })} disabled={closeTableMut.isPending} className="flex-1 py-3 bg-monaco-red rounded-xl text-white text-sm font-medium">{closeTableMut.isPending ? 'Cerrando...' : 'Confirmar'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center px-6">
          <div className="bg-monaco-card rounded-3xl p-6 w-full max-w-xs border border-white/10 space-y-4">
            ...
            <div className="space-y-2">
              ...
              {prizeType === 'custom' && <input value={prizeDesc} onChange={e => setPrizeDesc(e.target.value)} placeholder="Describe..." className="w-full bg-monaco-black border border-white/10 rounded-xl px-3 py-2.5 text-sm text-monaco-white placeholder-monaco-silver/40 focus:outline-none focus:border-monaco-red/50" />}
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={() => assignPrizeMut.mutate({ userId: assigningPrize.user_id, matchId: assigningPrize.match_id, prizeType, description: prizeDesc, tableNumber: assigningPrize.table_number })} disabled={assignPrizeMut.isPending} className="flex-1 py-3 bg-monaco-red rounded-xl text-white text-sm font-medium">{assignPrizeMut.isPending ? 'Asignando...' : 'Asignar'}</button>
            </div>
          </div>
        </div>
      )}
...
function KPI({ label, value, color }) {
  const bg = color === 'green' ? 'bg-green-500/10 border-green-500/20' : color === 'yellow' ? 'bg-yellow-500/10 border-yellow-500/20' : color === 'silver' ? 'bg-white/5 border-white/10' : 'bg-monaco-red/10 border-monaco-red/20'
  const tc = color === 'green' ? 'text-green-400' : color === 'yellow' ? 'text-yellow-400' : color === 'silver' ? 'text-monaco-white' : 'text-monaco-red'
  return (
    <div className={`rounded-2xl px-2 py-2.5 text-center border ${bg}`}>
      <p className={`text-2xl font-display leading-none ${tc}`}>{value}</p>
  ...
  return (
    <div className="card border-green-500/30 bg-green-500/5 space-y-3 mb-3">
      <div className="flex items-center justify-between">
        ...
        <span className="flex items-center gap-1 text-[10px] text-green-400"><span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" /> EN VIVO</span>
      </div>
      ...
      <div className="border-t border-white/5 pt-3">
        <p className="text-[10px] text-monaco-silver uppercase mb-2 flex items-center gap-1"><Zap size={10} className="text-yellow-400" /> Actualizar marcador</p>
  ...
  return (
    <div className="card space-y-3 mb-3">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => onToggleBet.mutate({ matchId: match.id, open: !match.betting_open })} className={`flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border transition-all ${match.betting_open ? 'bg-green-500/20 border-green-500/30 text-green-400' : 'bg-white/5 border-white/10 text-monaco-silver'}`}>
  ...
  return (
    <div className="card mb-2">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-[10px] text-monaco-silver bg-white/5 px-2 py-0.5 rounded-full">Finalizado</span>
      </div>
      <div className="flex items-center gap-3">
        ...
        <div className="px-3 py-1 bg-monaco-black rounded-xl border border-white/10">
          <span className="text-monaco-red font-display text-lg">{match.home_score} — {match.away_score}</span>
        </div>
...
function NightWinnerCard({ onAssign }) {
  const { data = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from('night_ranking').select('*').eq('night_date', today).order('night_points', { ascending: false }).limit(5)
      if (error) throw error
      return data ?? []
    },
    refetchInterval: 15000
  ...
  return (
    <div className="card border-yellow-500/20 space-y-3">
      ...
      {data.map((p, i) => (
        <div key={p.id} className={`flex items-center gap-3 py-2 px-3 rounded-xl ${i === 0 ? 'bg-yellow-500/10 border border-yellow-500/20' : ''}`}>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <span className={`font-display text-sm ${i === 0 ? 'text-yellow-400' : 'text-monaco-red'}`}>{p.night_points} pts</span>
            {i === 0 && (
              <button onClick={() => onAssign({ user_id: p.id, match_id: null, table_number: null, profiles: { full_name: p.full_name } })} className="flex items-center gap-1 px-2 py-1 rounded-lg bg-yellow-500/20 border border-yellow-500/30 text-yellow-400 text-[10px]">
                <Gift size={10} /> Premio
              </button>
            )}
          </div>
...
function TournamentPrizeCard({ user, qc }) {
  const [pt, setPt] = useState('vip_table')
  ...
  const { data: top } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('profiles').select('id, full_name, avatar_url, total_points, total_correct').eq('role', 'user').order('total_points', { ascending: false }).limit(1).maybeSingle()
      return data
    },
  ...
  const { data: ep } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('tournament_prizes').select('*, profiles(full_name)').not('winner_user_id', 'is', null).order('created_at', { ascending: false }).limit(1).maybeSingle()
      return data
    }
  })
  ...
  const am = useMutation({
    mutationFn: async ({ winnerId, type, desc }) => {
      const { error } = await supabase.from('tournament_prizes').insert({ prize_type: type, prize_description: desc, winner_user_id: winnerId, awarded_at: new Date().toISOString(), created_by: user.id })
      if (error) throw error
    },
    onSuccess: () => { toast.success('¡Premio asignado!'); qc.invalidateQueries({ queryKey: ['tournament-prize'] }); setAssigning(false) },
    ...
    <div className="card border-yellow-400/30 bg-yellow-400/5 space-y-3">
      ...
      {ep && (
        <div className="flex items-center gap-2 py-2 px-3 rounded-xl bg-green-500/10 border border-green-500/20">
          ...
          <p className="text-green-400 text-xs">Premio asignado a {ep.profiles?.full_name}</p>
        </div>
      )}
      {!assigning ? (
        <button onClick={() => setAssigning(true)} disabled={!!ep} className="w-full py-2.5 bg-yellow-500/20 border border-yellow-500/30 rounded-xl text-yellow-400 text-sm font-medium disabled:opacity-40">{ep ? 'Ya entregado' : 'Asignar premio'}</button>
      ) : (
        <div className="space-y-2">
          {[{ key: 'vip_table', label: 'Mesa VIP + botella' }, { key: 'bottle', label: 'Botella premium' }, { key: 'custom', label: 'Personalizado' }].map(p => (
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => am.mutate({ winnerId: top?.id, type: pt, desc: pd })} disabled={!top || am.isPending} className="flex-1 py-2.5 bg-yellow-500/20 border border-yellow-500/30 rounded-xl text-yellow-400 text-sm font-medium disabled:opacity-50">{am.isPending ? '...' : '🏆 Confirmar'}</button>
          </div>
        </div>
      )}
    </div>
...
function GrandPrizeConfig({ user }) {
  ...
  const { data: gp } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('tournament_prize_config').select('*').order('created_at', { ascending: false }).limit(1).maybeSingle()
      return data
    }
  })
  ...
  const sm = useMutation({
    mutationFn: async ({ type, desc }) => {
      ...
      const { error } = await supabase.from('tournament_prize_config').insert({ prize_type: type, prize_description: desc, created_by: user.id })
      if (error) throw error
    },
    onSuccess: () => { toast.success('Premio mayor configurado'); qc.invalidateQueries({ queryKey: ['grand-prize-config'] }); setEditing(false) },
    onError: (e) => toast.error(e.message)
  })

  const opts = [
    { key: 'bottle_prem', label: 'Botella premium' },
    { key: 'vip_table', label: 'Mesa VIP + botella' },
    { key: 'vip_night', label: 'Noche VIP completa' },
    { key: 'bottle_exclusive', label: 'Botella exclusiva + mesa VIP' },
    { key: 'party_pack', label: 'Pack fiesta' },
    ...
    <div className="card border-yellow-400/30 bg-yellow-400/5 space-y-3">
      ...
      {gp && !editing && (
        <div className="flex items-center gap-3 p-3 rounded-xl bg-yellow-500/10 border border-yellow-500/20">
          <div className="w-10 h-10 rounded-xl bg-yellow-500/20 flex items-center justify-center flex-shrink-0"><Trophy size={20} className="text-yellow-400" /></div>
          <div className="flex-1"><p className="text-monaco-white text-sm font-medium">🏆 {gp.prize_description || gp.prize_type}</p><p className="text-monaco-silver text-xs mt-0.5">Para el jugador con más puntos al final</p></div>
        </div>
      )}
      {!gp && !editing && <p className="text-monaco-silver text-xs text-center py-2">No has configurado el premio mayor aún</p>}
      {!editing ? (
        <button onClick={() => { if (gp) { setPt(gp.prize_type); setPd(gp.prize_description ?? '') } setEditing(true) }} className="w-full py-2.5 bg-yellow-500/20 border border-yellow-500/30 rounded-xl text-yellow-400 text-sm font-medium">{gp ? 'Cambiar premio mayor' : 'Configurar premio mayor'}</button>
      ) : (
        <div className="space-y-2">
          {opts.map(p => (
            <button key={p.key} onClick={() => { setPt(p.key); if (p.key !== 'custom') setPd(p.label) }} className={`w-full py-2 rounded-xl text-sm text-left px-3 transition-all ${pt === p.key ? 'bg-yellow-500/20 border border-yellow-500/30 text-yellow-400' : 'bg-white/5 text-monaco-silver border border-white/10'}`}>{p.label}</button>
          ))}
          ...
          <div className="flex gap-2">
            <button onClick={() => setEditing(false)} className="flex-1 py-2.5 bg-white/5 border border-white/10 rounded-xl text-monaco-silver text-sm">Cancelar</button>
            <button onClick={() => sm.mutate({ type: pt, desc: pd })} disabled={sm.isPending} className="flex-1 py-2.5 bg-yellow-500/20 border border-yellow-500/30 rounded-xl text-yellow-400 text-sm font-medium disabled:opacity-50">{sm.isPending ? '...' : '🏆 Guardar'}</button>
          </div>
        </div>
      )}
