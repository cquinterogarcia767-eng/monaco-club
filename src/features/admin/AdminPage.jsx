...
async function setMatchLive(matchId) {
  const { error } = await supabase.from('matches')
    .update({ status: 'live', betting_open: false }).eq('id', matchId)
...
async function addPrizeConfig({ prizeType, description, quantity }) {
  const { error } = await supabase.from('prize_config').insert({
    night_date: getToday(), prize_type: prizeType,
    prize_description: description, quantity,
  })
  if (error) throw error
}

async function removePrizeConfig(id) {
  const { error } = await supabase.from('prize_config')
...
export default function AdminPage() {
  ...
  const [newMatch, setNewMatch]               = useState({
    home_team: '', away_team: '', home_flag: '',
    away_flag: '', group_name: '', match_date: ''
  })
  ...
  function handleFinish(matchId) {
    ...
    if (!s || s.home === '' || s.away === '' || s.home === undefined || s.away === undefined)
      return toast.error('Ingresa el marcador final')
    finishMut.mutate({ matchId, homeScore: Number(s.home), awayScore: Number(s.away) })
  }
...
async function handleAddWaiter() {
  ...
  try {
    ...
    console.log('userId:', userId, 'error:', rpcError)
    ...
    if (updateError) return toast.error('Error actualizando rol: ' + updateError.message)

    toast.success('Mesero agregado correctamente')
    qc.invalidateQueries({ queryKey: ['waiters'] })
    setWaiterEmail('')
  } catch (e) {
    console.error('handleAddWaiter error:', e)
      ...
      <div className="px-4 py-6 space-y-4">
        ...
        {tab === 'dashboard' && (
          <div className="space-y-5">
              ...
              <div>
                ...
                {liveMatches.map(match => (
                  <LiveCard
                    ...
                    onScoreChange={(side, v) => setLiveScores(s => ({
                      ...s, [match.id]: { ...s[match.id], [side]: v }
                    }))}
            ...
            {/* Stats */}
            <p className="section-label flex items-center gap-2">
              <BarChart3 size={12} /> Estadísticas del torneo
            </p>
                    ...
                    <div key={p.id} className={`card flex items-center gap-3
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-monaco-silver text-xs">
                          {p.total_correct} aciertos · {p.total_bets} apuestas
                        </p>
                      </div>
                      <span className="text-monaco-red font-display text-lg flex-shrink-0">
                        {p.total_points}
                      </span>
                    </div>
                  ))}
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <button key={item.tab} onClick={() => setTab(item.tab)}
                  className="card flex items-center gap-3 p-3 active:scale-95 transition-all
                             hover:border-white/10">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0
                    ${item.color === 'red'    ? 'bg-monaco-red/20'  :
                      item.color === 'yellow' ? 'bg-yellow-500/20'  :
                      item.color === 'green'  ? 'bg-green-500/20'   : 'bg-blue-500/20'}`}>
                    <item.icon size={16} className={
                      item.color === 'red'    ? 'text-monaco-red'  :
        ...
        {tab === 'mesas' && (
          <div className="space-y-4">
            ...
            <div className="card border-monaco-red/20 space-y-3">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-monaco-black border border-white/10 rounded-xl
                             px-3 py-2.5 text-sm text-monaco-white placeholder-monaco-silver/40
                             focus:outline-none focus:border-monaco-red/50" />
                <button
                  onClick={() => {
                    if (!newTableNum) return toast.error('Ingresa el número')
                    createTableMut.mutate(Number(newTableNum))
                    setNewTableNum('')
                  }}
                  disabled={!newTableNum || createTableMut.isPending}
                  className="px-5 py-2.5 bg-monaco-red rounded-xl text-white text-sm
            ...
            {activeTables.length > 0 && (
              <div>
                <p className="section-label flex items-center gap-2">
                  <span className="w-2 h-2 bg-monaco-red rounded-full animate-pulse" />
                  Mesas ocupadas
                </p>
                  ...
                  return (
                    <div key={table.id} className="card border-monaco-red/20 mb-3 space-y-3">
                      ...
                      {tableUsers.map(session => (
                        <div key={session.id}
                          className="flex items-center gap-2.5 pt-2 border-t border-white/5">
                          <div className="w-8 h-8 rounded-full bg-monaco-red/20 border border-monaco-red/30
                                          flex items-center justify-center text-xs text-monaco-red flex-shrink-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-monaco-white text-xs font-medium truncate">
                              {session.profiles?.full_name ?? 'Usuario'}
                            </p>
            ...
            {freeTables.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {freeTables.map(t => (
                    <div key={t.id}
                      className="card aspect-square flex flex-col items-center
                                 justify-center gap-1 relative group">
                      ...
                      <button
                        onClick={() => deleteTableMut.mutate(t.id)}
            ...
            {tables.length === 0 && (
              <div className="card text-center py-10">
                ...
                <p className="text-monaco-silver/50 text-xs mt-1">Crea la primera mesa arriba</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            <div className="card border-monaco-red/20 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <div key={f.key} className="space-y-1">
                    ...
                    <input
                      value={newMatch[f.key]}
                      onChange={e => setNewMatch(m => ({ ...m, [f.key]: e.target.value }))}
                      placeholder={f.placeholder}
                      className="w-full bg-monaco-black border border-white/10 rounded-xl
                                 px-3 py-2 text-sm text-monaco-white placeholder-monaco-silver/30
                                 focus:outline-none focus:border-monaco-red/50" />
                  </div>
                ))}
              </div>
              <div className="space-y-1">
                ...
                <input value={newMatch.group_name}
                  onChange={e => setNewMatch(m => ({ ...m, group_name: e.target.value }))}
              ...
              <div className="space-y-1">
                ...
                <input type="datetime-local" value={newMatch.match_date}
                  onChange={e => setNewMatch(m => ({ ...m, match_date: e.target.value }))}
                  className="w-full bg-monaco-black border border-white/10 rounded-xl
                             px-3 py-2 text-sm text-monaco-white
                             focus:outline-none focus:border-monaco-red/50" />
              </div>
              <button onClick={handleCreateMatch} disabled={createMatchMut.isPending}
                ...
                {upcomingMatches.map(match => (
                  <div key={match.id} className="card space-y-3 mb-3">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        onClick={() => toggleBetMut.mutate({
                          matchId: match.id, open: !match.betting_open
                        })}
                        className={`flex items-center gap-1 text-[10px] px-2 py-0.5
                                    rounded-full border transition-all
                          ${match.betting_open
                            ? 'bg-green-500/20 border-green-500/30 text-green-400'
                            : 'bg-white/5 border-white/10 text-monaco-silver'}`}>
                        {match.betting_open
                          ? <><Unlock size={8} /> Apuestas abiertas</>
                          : <><Lock   size={8} /> Apuestas cerradas</>}
                      </button>
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2 mx-2">
                        <input type="number" min="0" max="20"
                          ...
                          placeholder="0"
                          className="w-12 h-12 bg-monaco-black border border-white/10 rounded-xl
                                     text-center text-xl text-monaco-white font-display
                                     focus:outline-none focus:border-monaco-red/50" />
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => liveMut.mutate(match.id)}
                      ...
                      <button onClick={() => handleFinish(match.id)}
                        disabled={finishMut.isPending}
                        className="flex-1 flex items-center justify-center gap-1.5 py-2.5
                                   bg-monaco-red text-white rounded-xl text-xs font-medium">
              ...
              <div>
                ...
                {finishedMatches.map(match => (
                  <div key={match.id} className="card flex items-center gap-2 mb-2 opacity-60">
                    ...
                    <span className="text-monaco-red font-display">
        ...
        {tab === 'premios' && (
          <div className="space-y-4">
            ...
            <div className="card border-yellow-500/20 space-y-3">
              <p className="text-monaco-white text-sm font-medium flex items-center gap-2">
                <Star size={14} className="text-yellow-400" /> Configurar premios de hoy
              </p>
              <div className="space-y-2">
                {[
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input type="number" min="1" max="20"
                    value={newPrize.qty}
                    onChange={e => setNewPrize(n => ({ ...n, qty: Number(e.target.value) }))}
                    className="w-16 bg-monaco-black border border-white/10 rounded-xl
                               px-2 py-1.5 text-sm text-monaco-white text-center
                               focus:outline-none focus:border-yellow-500/50" />
                </div>
                <button
                  ...
                  {prizeConfig.map(p => (
                    <div key={p.id} className="flex items-center gap-2">
                      <span className="text-yellow-400 text-xs flex-1">
                        {p.prize_description || p.prize_type} × {p.quantity}
                      </span>
            ...
            {/* Ganadores */}
            <p className="section-label flex items-center gap-2">
              <Crown size={12} className="text-yellow-400" /> Ganadores esta noche
            </p>
            {winners.length === 0 && (
              <div className="card text-center py-10">
                <Trophy size={28} className="text-monaco-silver/20 mx-auto mb-2" />
                <p className="text-monaco-silver text-sm">Aún no hay ganadores</p>
              </div>
            )}
            {winners.map(bet => (
              <div key={bet.id} className="card border-monaco-red/20 mb-2">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-monaco-red/20 border border-monaco-red/30
                                  flex items-center justify-center text-monaco-red font-display flex-shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-monaco-silver text-xs">
                      {bet.matches?.home_flag} {bet.predicted_home}—{bet.predicted_away} {bet.matches?.away_flag}
                      {' '}· +{bet.points_earned} pts
                    </p>
                  </div>
                  <button
                    onClick={() => { setAssigningPrize(bet); setPrizeType('discount_20') }}
          ...
          <div className="space-y-4">
            ...
            <div className="card border-yellow-500/20 space-y-3">
              ...
              <p className="text-monaco-silver text-xs leading-relaxed">
                El mesero debe registrarse primero en la app. Luego ingresa su email para darle acceso.
              </p>
              <input value={waiterEmail} onChange={e => setWaiterEmail(e.target.value)}
                ...
                className="w-full bg-monaco-black border border-white/10 rounded-xl
                           px-3 py-2.5 text-sm text-monaco-white placeholder-monaco-silver/40
                           focus:outline-none focus:border-yellow-500/50" />
              <button onClick={handleAddWaiter} className="btn-primary text-sm py-2.5">
                Asignar rol de mesero
              </button>
            </div>
            ...
            {waiters.length > 0 && (
              <div>
                <p className="section-label flex items-center gap-2">
                  <Users size={12} /> Meseros activos ({waiters.length})
                </p>
                {waiters.map(w => (
                  <div key={w.id} className="card flex items-center gap-3 mb-2">
                    <div className="w-9 h-9 rounded-full bg-yellow-500/20 border border-yellow-500/30
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="text-[10px] bg-yellow-500/20 text-yellow-400
                                       border border-yellow-500/30 px-1.5 py-0.5 rounded-full">
                        Mesero
                      </span>
                    </div>
                    <button onClick={() => removeWaiterMut.mutate(w.id)}
                      className="p-2 rounded-lg bg-white/5 border border-white/10 text-monaco-silver">
                      <Trash2 size={14} />
                    </button>
                  </div>
                ))}
            ...
            {/* Todos los clientes */}
            <p className="section-label flex items-center gap-2">
              <Users size={12} /> Clientes ({allUsers.filter(u => u.role === 'user').length})
            </p>
            {allUsers.filter(u => u.role === 'user').length === 0 && (
              <div className="card text-center py-8">
                <p className="text-monaco-silver text-sm">No hay clientes registrados</p>
              </div>
            )}
            {allUsers.filter(u => u.role === 'user').map((u, i) => (
              <div key={u.id} className="card flex items-center gap-3 mb-2">
                <span className="text-monaco-silver/40 text-xs w-5 text-center flex-shrink-0">
                  {i + 1}
                </span>
                <div className="w-8 h-8 rounded-full bg-monaco-red/20 border border-monaco-red/30
                                flex items-center justify-center text-xs text-monaco-red flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-monaco-silver text-xs">
                    {u.total_correct} aciertos · {u.total_bets} apuestas
                  </p>
                </div>
                <span className="text-monaco-red font-display text-sm flex-shrink-0">
                  {u.total_points} pts
                </span>
              </div>
            ))}
        ...
        {tab === 'ajustes' && (
          <div className="space-y-4">
            ...
            <div className="card border-red-500/20 space-y-3">
              ...
              <p className="text-monaco-silver text-xs leading-relaxed">
                Estas acciones afectan a todos los usuarios. Úsalas con cuidado.
              </p>
              <button
                onClick={async () => {
                  if (!confirm('¿Cerrar TODAS las mesas activas esta noche?')) return
                  for (const t of activeTables) {
                    await closeTable({ tableNumber: t.table_number, closedBy: user.id })
                  }
                  qc.invalidateQueries({ queryKey: ['tables'] })
                  qc.invalidateQueries({ queryKey: ['sessions'] })
                  toast.success('Todas las mesas cerradas')
                }}
                className="w-full py-3 bg-red-500/10 border border-red-500/30
                           rounded-xl text-red-400 text-sm font-medium">
          ...
          <div className="bg-monaco-card rounded-3xl p-6 w-full max-w-xs
            ...
            <div className="text-center">
              ...
              <p className="text-monaco-silver text-xs mt-2 leading-relaxed">
                Se bloquearán todos los usuarios de esta mesa y quedará disponible.
              </p>
            </div>
            <div className="flex gap-2">
              ...
              <button
                onClick={() => closeTableMut.mutate({ tableNumber: closingTable, closedBy: user.id })}
          ...
          <div className="bg-monaco-card rounded-3xl p-6 w-full max-w-xs
            ...
            <div className="text-center">
              ...
              <p className="text-monaco-silver text-xs mt-1">
                {assigningPrize.profiles?.full_name}
              </p>
            </div>
            <div className="space-y-2">
              {[
            ...
            <div className="flex gap-2">
              <button onClick={() => { setAssigningPrize(null); setPrizeDesc('') }}
                className="flex-1 py-3 bg-white/5 border border-white/10
                           rounded-xl text-monaco-silver text-sm">
                Cancelar
              </button>
              <button
                onClick={() => assignPrizeMut.mutate({
                  userId:      assigningPrize.user_id,
                  matchId:     assigningPrize.match_id,
                  prizeType,
                  description: prizeDesc,
                  tableNumber: assigningPrize.table_number,
                })}
    ...
    <div className="card border-green-500/30 bg-green-500/5 space-y-3 mb-3">
      ...
      <div className="flex items-center justify-center gap-3">
        ...
        <div className="px-4 py-1.5 bg-monaco-black rounded-xl border border-green-500/30">
          <span className="text-green-400 font-display text-xl">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={onUpdate} disabled={isPending}
            className="flex-1 flex items-center justify-center gap-1.5 py-3
                       bg-green-500/20 border border-green-500/30 rounded-xl
                       text-green-400 text-xs font-medium">
